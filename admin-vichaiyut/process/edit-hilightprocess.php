...
if(isset($_POST['btn-save']))
{
	...
	if(isset($_FILES["file"]["type"]) && $_FILES["file"]["type"] <> "")
	{
		...
		{ 
			...
			$stmt = $db_con->prepare("UPDATE hilight SET link =:link,publish_date = :publish_date,path_img = :path_img WHERE ID=:id");
...
else
{
	...
	try
		{ 
			...
			$stmt = $db_con->prepare("UPDATE hilight SET link =:link,publish_date = :publish_date WHERE ID=:id");
			$stmt->bindParam(':link', $link);
			$stmt->bindParam(':publish_date', $publish_date);
			$stmt->bindParam(':id', $id);
			$stmt->execute();
